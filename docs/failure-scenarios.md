...
> 작성일: 2026-06-19 · 갱신: 2026-07-14(명시 MCP 세션 격리·scoped action ownership 반영) · 범위: ssuMCP 구현된 resilience 패턴 기준 · 관련 ADR: 0021, 0022, 0015, 0047, 0059, 0086, 0097, 0098
...
`action_audit` 테이블의 상태 머신이 이를 막는다. `ActionStatus` enum의 lifecycle은 `PENDING → EXECUTING → SUCCESS|FAILED`이고, `PENDING`은 `EXPIRED`(TTL 경과) 또는 `SUPERSEDED`(같은 MCP session owner·action type·target key의 새 prepare)로도 종결된다. terminal outcome 상세(race/auth/upstream/timeout/partial)는 별도 `outcome_code` 문자열(`FAILURE_RACE`, `PARTIAL_FAILURE` 등)에 남는다.

```
prepare_reserve_library_seat()
  │
  ├─→ 같은 (MCP session owner, action type, target key)의 기존 PENDING만 SUPERSEDED로 원자 전이
  └─→ action_audit 행 INSERT (actionId, PENDING, dry_run_preview, expires_at)

confirm_action(action_id?)
  │
  ├── claimByIdAndMcpSessionAndStatus: WHERE id=action_id AND mcp_session_id=owner AND status='PENDING' FOR UPDATE
...
**두 번째 confirm은 무조건 실패**: `SELECT FOR UPDATE`가 이미 EXECUTING/SUCCESS인 행을 PENDING으로 보지 않기 때문이다. **stale/foreign 액션 실행도 차단**: action은 정확한 MCP session owner로 claim하며, prepare는 같은 대상의 이전 PENDING만 supersede한다. 서로 다른 대상이 여러 개면 `confirm_action`은 추측하지 않고 `ACTION_CONFLICT`로 `action_id`를 요구한다(ADR 0086·0098).
...
## 6. 비동기 confirm 응답 vs worker 성공 (이중 상태 사고)

### 무엇이 잘못되는가

과거 예약 confirm은 worker 결과를 동기 대기했다. timeout을 "실패"로 단정하면 worker가 뒤늦게 실제 좌석을 잡을 수 있고, 동기 경로가 audit를 FAILED로 닫은 뒤 worker가 성공해 **API는 실패·좌석은 예약·감사 로그는 오류**가 되는 double-state가 생긴다. 현재 MCP confirm은 즉시 intent 접수만 반환하지만, 웹/worker 경로도 같은 audit 규칙을 유지해야 한다.

### 우리가 하는 일 (ADR 0059, 사건 16)

- **MCP는 즉시 접수**: `confirm_action`의 예약 경로는 worker 폴링 없이 intent ID를 포함한 비종단 접수 응답을 반환한다. 결과 조회는 `get_library_wait_status`/SSE가 담당한다.
- **타임아웃은 비종단**: 동기 웹 경로가 관찰을 해야 할 경우에도 audit를 terminal FAIL로 닫지 않는다. audit는 `EXECUTING`에 남기고 비종단 `PROCESSING`만 반환한다.
...
비종단 audit를 `EXECUTING`에 남기므로 미완 상태가 잠시 보일 수 있으나, worker/expire/cancel 경로가 종결한다. "응답 상태와 worker의 비즈니스 진실 분리"의 트레이드오프다.
...
| confirm 접수 vs worker 성공 | MCP는 즉시 intent 접수, 웹 관찰 경로는 비종단 | worker SoT finalize (ADR 0059) | intent 상태 조회/SSE, audit 정확 |
