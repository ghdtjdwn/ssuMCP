...
이건 단일 서비스 보안이 아니라 **"폴리글랏(Java/Kotlin + Python + TypeScript) 마이크로서비스에서 보안 경계를 어떻게 동일하게 유지하는가"**의 문제다. 한 서비스의 통제가 주변 서비스에 빠지면 가장 약한 경계가 전체 시스템의 노출면이 되므로, 공통 위협 모델에 맞춰 통제를 정렬해야 했다.
