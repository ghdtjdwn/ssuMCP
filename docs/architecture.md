...
기본 구현은 `RedissonLibraryIntentStatusBus`(RTopic, at-most-once·비영속)다. `ssuai.kafka.intent-bus.enabled=true`면 **`KafkaLibraryIntentStatusBus`**로 승격된다([ADR 0091](adr/0091-reservation-intent-bus-kafka.md)): 토픽 `library.reservation.events.v1`에 key=intentId로 발행(순서 보장), **포드마다 유일한 consumer group + `auto.offset.reset=latest`**로 브로드캐스트 fan-out. publish는 단일 스레드 bounded executor에 offload되어 relay 스레드를 절대 막지 않고(fail-open), 브로커가 죽어도 예약은 outbox로 진행되고 알림만 지연된다. 별도 dormant 플래그라 코드 머지는 런타임 영향이 없고, cutover/롤백은 플래그 하나로 가역적이다. 메트릭 `library.intent.bus.event{result=sent|dropped_queue_full|dropped_error}`.
