...
비공개 도구는 MCP loopback을 거치면 servlet thread가 달라져 `ThreadLocal` 인증 문맥을 전달할 수 없으므로 현재 채팅 thread에서 서비스를 직접 호출해야 한다. 따라서 이 변경은 실행 방식을 바꾸는 기능 개선이 아니라, 동기 인프로세스 실행이라는 기존 제약을 유지하면서 책임과 의존성의 소유자만 옮기는 순수 리팩터링이다.
