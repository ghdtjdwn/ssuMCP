...
프로젝트의 "영구 무료" 서사(ADR 0007)를 지키는 한 유료 전환은 최후 수단이어야 하고, 그 전에 **2 OCPU/12 GB 단일노드**를 실제 운영 예산으로 못 박아 전 스택을 right-size해야 한다.
...
가장 쉬운 해법이지만 ADR 0007의 "영구 무료" 서사를 정면으로 훼손한다. 개인 프로젝트로서 "취업 후에도 비용 없이 계속 살아있는 데모"라는 스토리가 이 결정 하나로 무너진다. 다만 완전히 폐기하지는 않는다 — 트림(아래 §4)까지 다 써도 안 되는 순간이 오면 그때 다시 꺼낼 옵션으로만 남겨둔다.
...
headroom을 다 써도 부족해지면(신규 컴포넌트 온보딩, 트래픽 스파이크 등) 아래 순서로 되돌린다. 앞 단계일수록 프로젝트 핵심 서사에서 먼 컴포넌트다.
