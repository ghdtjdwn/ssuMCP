...
1. **저작권·ToS 리스크**: 강의 MP4를 서버에서 직접 다운로드·처리하는 행위는 숭실대 LMS 이용약관 위반
   소지가 있고, 권한이 있는 자료라도 서버가 원본 영상을 복제·처리하는 범위가 제품 목적에 비해 과도하다.
...
| 전체 제거 + 비영상 자료 ZIP (ADR-0033) | 저작권 위험을 줄이면서 사용자가 필요한 비영상 자료 내보내기를 유지한다. 채택. |
...
- 설명 불가한 저작권 리스크는 제품 신뢰와 운영 지속성을 해친다.
