...
>
> **2026-07-10 후속**: 이 ADR은 "분당 수백 건·<5 소비자" 규모에서 Kafka를 **미도입**하고 graduate 트리거를 명문화한 결정이다. 이후 시나리오가 "전교생 수만 명 공식 캠퍼스 MCP"로 상향되어 그 트리거가 충족 → 계획대로 Kafka로 승격했다(0089/0090/0091). 아래 본문은 당시 용량과 요구사항에 따른 결정 근거로 보존한다.

---

## 배경

현재 mp의 이벤트 전파는 **Kafka 없이** 두 메커니즘으로 동작한다.
...
이 ADR은 Kafka 추가가 현재 처리량·소비자 수·재처리 요구에 맞는지 검토하고, 전환 기준을 기록한다.

## 결정

**Kafka를 추가하지 않는다.** 현행 Redisson topic + LISTEN/NOTIFY를 유지하고, 대신 두 가지 운영 기준을 명문화한다.
...
- **운영 부담**: 단일 Oracle ARM 노드에 ZooKeeper-less라도 브로커 + 토픽 운영·모니터링·리밸런싱 비용이 든다. Confluent 자체 가이드도 소규모에서 Kafka 운영을 "0.5~2 FTE"로 본다. 단일노드 프로젝트에 부적합.
- **복잡도 대비 이점 부족**: 현재 처리량과 소비자 수에서는 브로커 운영·모니터링·리밸런싱 비용이 내구성·리플레이 이점보다 크다.
