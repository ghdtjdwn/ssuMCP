...
>
> **2026-07-10 후속**: 이 ADR은 "분당 수백 건·<5 소비자" 규모에서 Kafka를 **미도입**하고 graduate 트리거를 명문화한 결정이다. 이후 시나리오가 "전교생 수만 명 공식 캠퍼스 MCP"로 상향되어 그 트리거가 충족 → 계획대로 Kafka로 승격했다(0089/0090/0091). 아래 본문은 **당시 근거**로 그대로 보존한다(기술 검토에서 "왜 처음엔 안 썼고 언제·어떻게 도입했나"의 전반부 근거).
...
"프로젝트에 Kafka를 넣어야 하나?"가 이 ADR의 질문이다. 무작정 추가가 아니라 **규모에 맞는 선택인지**를 웹서치로 검증하고 결정을 기록한다(철칙2).
...
- **프로젝트 역효과**: "왜 이 규모에 Kafka를?"라는 과설계 신호가 될 수 있다. 오히려 **규모 판단을 보여주는 게** 더 강한 신호다.
