...
백엔드를 단일 pod에서 멀티 pod(k3s replica > 1)로 늘리려면, replica=1을 전제로 동작하던 두 경계를 먼저 바로잡아야 했다.

**공유 예산 경계 — per-pod 카운터가 멀티 pod에서 N배 누수.** 두 곳이 동일한 문제를 안고 있었다.
...
2. `PyxisResilience`(ADR 0029) — 학교 시스템(oasis.ssu.ac.kr) 보호를 위한 Resilience4j `RateLimiter`(read 5/s, write 2/s)도 마찬가지로 pod-local이다. replica가 늘면 우리가 공유하는 학교 시스템에 가하는 실제 부하가 `5 × N`/s로 커져 egress IP 차단 리스크가 커진다. 여기에 더해 기존 캡은 "전체 트래픽 5/s"만 있을 뿐 "사용자 1명이 그 5/s를 혼자 다 쓰는 것"을 막는 장치가 없었다 — 학교 보호 캡과 사용자 공정성 캡이 하나로 뭉쳐 있었다.

**클라이언트 IP 경계 — XFF 좌측 신뢰로 인한 버킷 회전.** `ClientIpResolver`는 `X-Forwarded-For`의 **좌측** 엔트리를 클라이언트 IP로 신뢰했다. k3s Traefik ingress는 자신이 본 peer 주소를 헤더 **우측**에 append하는 구조라, 좌측은 클라이언트가 원하는 대로 채울 수 있는 완전히 위조 가능한 입력이다. 즉 공격자는 매 요청마다 `X-Forwarded-For: <임의값>`을 바꿔가며 자신의 rate-limit 버킷을 회전시켜 인바운드 한도를 무력화할 수 있었다.
...
ADR 0024가 이미 "Redis는 correctness가 아니라 efficiency/scale-out 보조 인프라"라는 원칙과 Redisson 4.5.0 의존성, graceful-degradation 컨벤션(WARN + metric + fallback)을 확립해 뒀다. 이 결정은 그 컨벤션을 인바운드 rate limit과 Pyxis 업스트림 캡까지 확장한다.

### 토큰버킷 vs 고정윈도우(인바운드 공유 한도)
...
Proxy Protocol(TCP 레벨에서 원본 클라이언트 IP를 전달)도 검토했으나 기각 — Traefik ingress 설정과 배포 매니페스트(`deploy/charts`) 변경이 필요하고, 이 결정의 범위는 애플리케이션 레이어 수정으로 한정했다. 대신 **trusted-hop 개수만큼 우측에서 신뢰**하는 방식을 택했다: `X-Forwarded-For`는 각 프록시가 자신이 본 peer 주소를 헤더 **우측에 append**하는 구조이므로, 신뢰하는 프록시 개수(`trustedProxyCount`, 기본 1 = Traefik만)를 알면 우측에서 그 개수만큼 뒤로 간 위치가 항상 진짜 클라이언트다. 공격자가 좌측에 몇 개를 덧붙이든(가짜 프록시 체인을 흉내내든) 우측 고정 위치는 오염시킬 수 없다. 헤더 엔트리 수가 `trustedProxyCount`보다 적으면(홉이 예상보다 적음 — 헤더 위조 또는 배포 변경) 안전하게 `getRemoteAddr()`로 폴백한다(추측하지 않는다).

## 선택

**인바운드 공유 예산(`SharedIpRateLimiter`)**: Redisson `RAtomicLong` 기반 고정윈도우 카운터로 `IpRateLimiter`와 동일한 알고리즘을 Redis에 구현. `ssuai.ratelimit.redis-enabled`(기본 `true`)로 켜져 있으며, replica=1에서는 로컬과 동일하게 동작하고 replica>1부터 실제 공유 효과가 생긴다 — **설정 변경 없이 정답이 되는 기본값**이다.

**Pyxis 공유·사용자 cap(`PyxisResilience.acquireDistributed`)**: 읽기/쓰기 각각에 대해 두 단계 Redisson `RRateLimiter`를 순서대로 확인한다.
...
**`ClientIpResolver`**: 일반 route는 `trustedProxyCount`(기본 1, `ssuai.ratelimit.trusted-proxy-count`)만큼 우측에서 위치를 계산하며, 인증된 proxy hop이 추가되는 환경에서는 설정값을 늘린다. 정책 초안 생성 route는 public caller가 XFF prefix를 위조해 bucket을 우회하지 못하도록 설정값과 무관하게 한 hop으로 고정한다([ADR 0102](0102-grounded-policy-review-copilot.md)).
...
- **정확도 vs 가용성**: Redis 장애 중에는 다시 "pod별 독립 카운터"로 돌아가므로, 그 짧은 창(window) 동안은 `한도 × N` 문제가 일시적으로 재발한다. 학교 시스템 보호라는 목적에서 보면 이 창이 짧고 드물다는 전제(Redis는 로컬 단일 pod, 영속성 없음 — ADR 0024 D1)로 감수 가능하다고 판단했다.
