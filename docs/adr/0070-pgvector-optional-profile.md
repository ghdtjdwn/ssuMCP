...
|---|---|
| 날짜 | 2026-06-30 |
| 상태 | Accepted — 옵션 프로파일로 ANN 경로 검증, prod 기본 경로 불변 |
...
현재 규모에서 ANN을 라이브 경로에 넣을 이점은 없지만, 코퍼스가 커질 때의 전환 경로와 PostgreSQL 확장·마이그레이션 호환성은 운영 전 분리된 환경에서 검증할 필요가 있다.

## 결정

**prod 기본 경로는 그대로 두고**, `pgvector` 프로파일에서만 활성화되는 ANN 경로를 추가해 **기능과 마이그레이션을 격리 검증**한다.
...
- **검증**: `PgVectorAnnIndexIT`가 **`pgvector/pgvector:pg16` Testcontainers**(SET B 베이스 재사용)로 V14 적용 + 768-d 벡터 3개 삽입 후 코사인 최근접이 기대 청크를 반환함을 단언. 라이브 RAG는 **건드리지 않는다**.

## 대안과 기각 이유

- **prod 통째 pgvector 교체** ❌ — prod에 확장이 없고, 규모상 불필요하며, 라이브 RAG(현재 정상 동작 중)를 바꾸는 회귀 위험만 크다.
- **외부 벡터DB(Pinecone/Qdrant/Weaviate)** ❌ — 인프라·비용·운영 표면이 증가하고 현재 단일 노드 운영 모델에 맞지 않는다.
- **라이브 스토어를 `@Primary`로 스왑(pgvector 프로파일)** △ — 가능하지만, 임베딩 **생성** 컨트랙트(`AcademicEmbeddingStore.embed`)와 벡터 **검색**(ANN)은 다른 관심사라 억지로 합치면 RRF 융합부까지 손대야 한다. 이번엔 **검색측 격리 컴포넌트(`PgVectorAnnIndex`)**로 기능만 검증하고, 라이브 RRF에 ANN을 끼우는 것은 코퍼스 규모와 성능 측정이 전환을 정당화할 때로 미룬다.
- 인덱스: **HNSW(cosine_ops)** ✓ vs IVFFlat △ — IVFFlat은 list 수 튜닝/학습이 필요하고 증분 삽입에 약하다. HNSW는 build-once·증분 친화.

## 프로덕션에 기본 적용하지 않은 이유

규모 판단이다. 217청크에서 인메모리 exact 코사인은 단순·정확·의존성 0이고, prod에 확장 도입·이중쓰기·HNSW 인덱스 유지 비용을 더해도 검색 품질이나 지연 이득이 사실상 없다. 현재 서빙 경로는 단순하게 유지하되, 코퍼스 증가 시 사용할 전환 경로만 분리된 프로파일과 통합 테스트로 검증한다.
...
- **superseded**: "prod엔 강제하지 않았다"는 "격리 검증 상태 그대로 둔다"가 아니라, "검증한 기능을 코퍼스 성장에 대비해 prod에 미리 켜 둔" 상태로 갱신해서 읽어야 한다(values.yaml 주석 참고).
...
현재 상태는 "확장은 prod에 이미 있고 켜져 있지만, 217청크 규모에서는 인메모리 검색이 충분해 서빙 경로를 아직 전환하지 않았다"로 정리한다.
