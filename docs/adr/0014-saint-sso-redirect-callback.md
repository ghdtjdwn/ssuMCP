...
- **No Spring Security.** `JwtAuthFilter` parses `Authorization: Bearer`
  ...
  This keeps the auth stack small and inspectable for the project.
  If the threat surface grows (CSRF tokens, role-based
...
The 2026-05-16 debugging result is summarized above with its regression
coverage.
