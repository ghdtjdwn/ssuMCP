...
공모전의 업무 개선·확산 가능성을 실제 제품 가치로 증명하려면 AI를 최종 결정자로 두지 않고, 반복적인
공개 정책 답변 초안 작성에는 사용하되 지정 검토와 정량 지표를 하나의 내구성 있는 흐름으로 연결해야 한다.
...
- 기존 `action_audit` 재사용: 학교 상태 변경의 prepare/confirm 감사 수명과 답변 검토 case의 상태·지표가
  달라 의미를 섞으므로 기각했다.
- 모델 confidence 백분율 저장: calibration 근거가 없고 심사·운영자에게 거짓 정밀도를 주므로 기각했다.
...
- 생성형 AI의 답변 confidence 대신 검증 가능한 review reason을 선택한 이유는 무엇인가?
- 두 reviewer가 동시에 claim할 때 optimistic lock과 409 계약은 어떤 정합성을 보장하는가?
- 업무 지표를 Prometheus label이 아니라 durable row 집계로 계산한 이유는 무엇인가?
- 사용자 query를 embedding에서 제외하면서도 검색 품질과 최신성을 어떻게 유지하는가?
