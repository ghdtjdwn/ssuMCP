...
class McpLibraryAuthControllerTests {
    ...
    void callbackWithInvalidStateReturns400() throws Exception {
        when(mcpAuthService.peekState("bad-state")).thenReturn(Optional.empty());
        ...
        verify(mcpAuthService, never()).consumeState(any());
    }
    ...
    void callbackWithProviderMismatchReturns400() throws Exception {
        ...
        when(mcpAuthService.peekState("state")).thenReturn(Optional.of(entry));
    ...
    @Test
    void callbackAuthFailureReturns401AndPreservesState() throws Exception {
        ...
        // State must NOT be consumed on credential failure so the user can retry
        verify(mcpAuthService, never()).consumeState(any());
