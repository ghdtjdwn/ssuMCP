...
public class LibraryAvailableSeatsMcpTool {
    ...
    @Tool(
            ...
            description = "숭실대학교 도서관 전체 열람실(7개)의 예약 가능 좌석을 실시간으로 조회합니다. "
                    ...
                    + "compact=true이면 좌석 ID 배열을 생략하고, offset/limit(최대 200)으로 "
                    + "열람실별 배열 크기를 제한할 수 있습니다. truncated/hasMore 메타데이터를 확인하세요. "
    ...
    public McpPrivateToolResponse<LibraryAllAvailableSeatsResponse> getLibraryAvailableSeats(
            @ToolParam(required = false, description = "true이면 열람실별 좌석 ID/번호 배열을 생략합니다.")
            Boolean compact,
            @ToolParam(required = false, description = "열람실별 이용 가능 좌석 배열 시작 위치(0 이상).")
            Integer offset,
            @ToolParam(required = false, description = "열람실별 최대 반환 좌석 수(1~200).")
            Integer limit,
                ...
                .map(principal -> {
                    ...
                    try {
                        LibraryAllAvailableSeatsResponse data =
                                availableSeatsService.getAllAvailableSeats(
                                        principal.providerSessionKey(), compact, offset, limit);
    ...
    public McpPrivateToolResponse<LibraryAllAvailableSeatsResponse> getLibraryAvailableSeats(
            String mcpSessionId) {
        return getLibraryAvailableSeats(false, 0, null, mcpSessionId);
    }
}
